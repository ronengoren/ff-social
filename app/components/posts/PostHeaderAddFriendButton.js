...
class PostHeaderAddFriendButton extends React.Component {
  ...
  render() {
    ...
    const isFriendshipDeclinded = friendshipStatus === friendshipStatusType.REJECTED;
    const text = requested || isFriendshipDeclinded ? I18n.t('feed.post_header.friendship_button.pending') : I18n.t('feed.post_header.friendship_button.add_friend');
    const color = requested || isFriendshipDeclinded ? flipFlopColors.b60 : flipFlopColors.green;
    if (postType === postTypes.SHARE) {
      return null;
    }
    const isRTLText = isRTL(text);
    return (
      <Text style={isRTLText ? styles.addFriendRTL : styles.addFriend} onPress={isFriendshipDeclinded ? null : this.toggleFriendshipRequest} color={color}>
        {text}
      </Text>
    );
  }
